...
def nextItems(itemsList, index, numberOfItems):
  nextItems = []

  for x in range(1,numberOfItems):
    if len(itemsList) >= (index + x + 1):
      nextItems.append(itemsList[index + x]) 
      
  return nextItems

def buildMimic(words):
  """ Builds the mimic_dict with the wordlist creating the list of words and its next 7 words"""
  mimic = {}

  for (index, value) in enumerate(words):
    if value not in mimic:
      mimic[str(value)] = nextItems(words, index, 8)
...
def mimic_dict(filename):
  ...
  mimic_dict = buildMimic(wordsList)

  return mimic_dict


def print_mimic(mimic_dict, word):
  """Given mimic dict and start word, prints (if possible) 200 random words."""
  text = []
  count = 200

  """
  if len(mimic_dict) < 200: 
    count = len(mimic_dict) - 1
  """

  for x in range(0,count):
    if word == "" or word not in mimic_dict:
      if len(mimic_dict.keys()) > 0:
        #print(mimic_dict.keys())
        word = random.choice(mimic_dict.keys())
      else:
        word = ""
      #print("Skipped space with new word: " + word)

    else:
      if len(mimic_dict.keys()) > 0:
        word = random.choice(mimic_dict.keys())
        if len(mimic_dict[word]) > 0:
          text.append(random.choice(mimic_dict[word]))
      else:
        word = ""
...
def print_mimic(mimic_dict, word):
  #Gven mimic dict and start word, prints 200 random words.
  # +++your code here+++
  # LAB(begin solution)
